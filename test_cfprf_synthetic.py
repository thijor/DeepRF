...
Script to test CFpRF on synthetic data.
"""
...
def run(root="/project/2420084.01", n_voxels=1024):
    ...
    # Set coarse fit grids
    h_grid = (-1, 1)
    s_grid = (1/stimuli.pixperdeg*2, stimuli.radius_deg/2)
    x_grid = (-stimuli.radius_deg/2, stimuli.radius_deg/2)
    y_grid = (-stimuli.radius_deg/2, stimuli.radius_deg/2)
    grids = (h_grid, s_grid, x_grid, y_grid)
    ...
    for i in range(n_voxels):
        ...
        # Coarse fit
        coarse_fit = brute(error_function, args=(bounds, x, train_generator.generate_prediction), ranges=grids, Ns=3, disp=False)
        ...
        print("voxel {:5d}/{} ctime: {:6.2f} ({:6.2f}) train: {:.2f} ({:.2f}) valid: {:.2f} ({:.2f})".format(1+i, n_voxels, ctime[i], np.mean(ctime[:1+i]), r_train[i], np.mean(r_train[:1+i]), r_valid[i], np.mean(r_valid[:1+i])))

    # Make output folder if it does not already exist
    if not os.path.exists(os.path.join(root, "derivatives", "cfprf")):
        os.makedirs(os.path.join(root, "derivatives", "cfprf"))

    # Save results
    np.savez(os.path.join(root, "derivatives", "cfprf", "results_synthetic.npz"), 
