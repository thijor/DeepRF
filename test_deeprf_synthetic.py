...
Script to test DeepRF on synthetic data using a CPU and passing individual voxels through the model to
...
def run(root="/project/2420084.01", n_voxels=1024, cat_dim="time", n_layers=50, n_dropout=1000):
    ...
    for i in range(n_voxels):
        ...
        print("voxel {:5d}/{} ctime: {:6.3f} ({:6.3f}) + {:6.3f} ({:6.3f}) train: {:.2f} ({:.2f}) valid: {:.2f} ({:.2f})".format(
    ...
    # Save results
    np.savez(os.path.join(root, "derivatives", model.name+"_"+cat_dim, "results_synthetic.npz"), 
        t_train=t_train, t_valid=t_valid, y_train=y_train, r_train=r_train, r_valid=r_valid, ctime=ctime, y_train_dropout=y_train_dropout, ctime_dropout=ctime_dropout)

if __name__ == "__main__":
    root = "/project/2420084.01"
    #root = "/Users/jordythielen/2420084.01"
    run(root=root, n_voxels=1024, cat_dim="chan", n_layers=50, n_dropout=0)
